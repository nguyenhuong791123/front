...
window.onload = function () {
    ...
    // set debug level
    SIPml.setDebugLevel((window.localStorage && window.localStorage.getItem('sc.ippbx.disable_debug') == "true") ? "error" : "info");
    // window.localStorage.setItem('sc.ippbx.disable_video', "true");
...
//     loadCallOptions();
    // window.localStorage.setItem('sc.ippbx.disable_video', chkCallMode.checked);
//   }
  ...
  //   window.localStorage.setItem('sc.ippbx.disable_video', chkCallMode.checked ? "true" : "false");
  //   loadCallOptions();
  // }
// }
...
function loadModeCall() {
  ...
  const mode = window.localStorage.getItem('sc.ippbx.disable_video');
  ...
  window.localStorage.setItem('sc.ippbx.disable_video', bDisableVideo ? "true" : "false");
//   setDailerBoxWidthHeight();
//   if(!mode) {
...
function loadCallOptions() {
    if (window.localStorage) {
        var s_value;
        if ((s_value = window.localStorage.getItem('sc.ippbx.phone_number'))) txtPhoneNumber.value = s_value;
        bDisableVideo = (window.localStorage.getItem('sc.ippbx.disable_video') == "true");
        trDivVideo.style.display = 'none';
        // loadModeCall(bDisableVideo);

        setVolumeMode(window.localStorage.getItem('sc.ippbx.volume_mode') == 'true');
        // chkCallMode.checked = bDisableVideo;
        // txtCallStatus.innerHTML = '<i>Video ' + (bDisableVideo ? 'disabled' : 'enabled') + '</i>';
        if(sRegistered) trDivVideo.style.display = (bDisableVideo)?'none':'grid';
    } else {
        window.localStorage.setItem('sc.ippbx.disable_video', 'false');
    }
}

function setVolumeMode(volumeMode) {
  vMode = volumeMode;
  window.localStorage.setItem('sc.ippbx.volume_mode', volumeMode ? "true" : "false")
...
//     if (window.localStorage) {
//         var s_value;
//         if ((s_value = window.localStorage.getItem('sc.ippbx.call.phone_number'))) txtPhoneNumber.value = s_value;
//         bDisableVideo = (window.localStorage.getItem('sc.ippbx.disable_video') == "true");
...
function saveCallOptions() {
    if (window.localStorage) {
        window.localStorage.setItem('sc.ippbx.phone_number', txtPhoneNumber.value);
        window.localStorage.setItem('sc.ippbx.disable_video', bDisableVideo ? "true" : "false");
    }
}
...
function loadCredentials() {
  if (window.localStorage) {
    ...
    var s_value;
    // if ((s_value = window.localStorage.getItem('sc.ippbx.display_name'))) txtDisplayName.value = s_value;
    if ((s_value = window.localStorage.getItem('sc.ippbx.impi'))) txtPrivateIdentity.value = s_value;
    if ((s_value = window.localStorage.getItem('sc.ippbx.impu'))) txtPublicIdentity.value = s_value;
    if ((s_value = window.localStorage.getItem('sc.ippbx.password'))) txtPassword.value = s_value;
    if ((s_value = window.localStorage.getItem('sc.ippbx.realm'))) txtRealm.value = s_value;
    if ((s_value = window.localStorage.getItem('sc.ippbx.websocket_server_url'))) txtWs.value = s_value;
    if ((s_value = window.localStorage.getItem('sc.ippbx.sip_outboundproxy_url'))) txtUdpProxy.value = s_value;
  } else {
...
function saveCredentials() {
  if (window.localStorage) {
    // window.localStorage.setItem('sc.ippbx.display_name', txtDisplayName.value);
    window.localStorage.setItem('sc.ippbx.impi', txtPrivateIdentity.value);
    window.localStorage.setItem('sc.ippbx.impu', txtPublicIdentity.value);
    window.localStorage.setItem('sc.ippbx.password', txtPassword.value);
    window.localStorage.setItem('sc.ippbx.realm', txtRealm.value);
    window.localStorage.setItem('sc.ippbx.websocket_server_url', txtWs.value);
    window.localStorage.setItem('sc.ippbx.sip_outboundproxy_url', txtUdpProxy.value);
...
function sipRegister() {
    ...
    try {
        ...
        oSipStack = new SIPml.Stack({
            ...
            enable_rtcweb_breaker: (window.localStorage ? window.localStorage.getItem('sc.ippbx.enable_rtcweb_breaker') == "true" : false),
            events_listener: { events: '*', listener: onSipEventStack },
            enable_early_ims: (window.localStorage ? window.localStorage.getItem('sc.ippbx.disable_early_ims') != "true" : true), // Must be true unless you're using a real IMS network
            enable_media_stream_cache: (window.localStorage ? window.localStorage.getItem('sc.ippbx.enable_media_caching') == "true" : false),
            bandwidth: (window.localStorage ? tsk_string_to_object(window.localStorage.getItem('sc.ippbx.bandwidth')) : null), // could be redefined a session-level
            video_size: (window.localStorage ? tsk_string_to_object(window.localStorage.getItem('sc.ippbx.video_size')) : null), // could be redefined a session-level
            sip_headers: [
...
function sipCall(s_type) {
    if (oSipStack && !oSipSessionCall && !tsk_string_is_null_or_empty(txtPhoneNumber.value)) {
        ...
        if (window.localStorage) {
            oConfigCall.bandwidth = tsk_string_to_object(window.localStorage.getItem('sc.ippbx.bandwidth')); // already defined at stack-level but redifined to use latest values
            oConfigCall.video_size = tsk_string_to_object(window.localStorage.getItem('sc.ippbx.video_size')); // already defined at stack-level but redifined to use latest values
        }
...
function onKeyUp(evt) {
  ...
  if (evt.keyCode == 27) {
    ...
    if (evt.keyCode == 65 || evt.keyCode == 86) { // A (65) or V (86)
      ...
      txtCallStatus.innerHTML = '<i>Video ' + (bDisableVideo ? 'disabled' : 'enabled') + '</i>';
      window.localStorage.setItem('sc.ippbx.disable_video', bDisableVideo);
    }
  }
}
...
function uiDisableCallOptions() {
  if (window.localStorage) {
    window.localStorage.setItem('sc.ippbx.disable_callbtn_options', 'true');
    uiBtnCallSetText('Call');
